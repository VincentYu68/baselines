...
def binary_search_curriculum(env, policy, anchor, direction, threshold, max_threshold, max_step):
    ...
    if (bound_performance - threshold) < np.abs(
                    threshold * 0.1) and bound_performance > threshold and bound_max > max_threshold:
...
def callback(localv, globalv):
    ...
    joblib.dump(save_dict, logger.get_dir() + '/policy_params_' + str(localv['env'].env.env.anchor_kp) + '_' + str(
